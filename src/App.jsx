...
function AdminPanel({items,setItems,employees,setEmployees,lines,setLines,showToast,reload}){
  ...
  const delLine=async id=>{try{await db.deleteLine(id);setLines(p=>p.filter(l=>l.id!==id));showToast("Removed.");}catch{showToast("Failed.","erro
  return(
    <div className="au">
      ...
      {tab==="planned"&&<div style={{color:"#4A4F65",padding:20}}>Loading planned orders tab…</div>}
      {tab==="users"&&<div style={{color:"#4A4F65",padding:20}}>Loading users tab…</div>}
      {tab==="items"&&<div style={{color:"#4A4F65",padding:20}}>Loading items tab…</div>}
      {tab==="employees"&&<div style={{color:"#4A4F65",padding:20}}>Loading employees tab…</div>}
      {tab==="lines"&&<div style={{color:"#4A4F65",padding:20}}>Loading lines tab…</div>}
    </div>
  );
}
